    ...
    public class ProtoBufInitialiser : IInitialiser
    {
        public void Initialise()
        {
            ...
            foreach (var t in primitives.Concat(customTypes))
                RuntimeTypeModel.Default[typeof(Standalone.Serialization.MessageParam)].AddSubType(messageCounter++, 
                    typeof(Standalone.Serialization.MessageParam<>).MakeGenericType(Type.GetType(t)));
        }
    }
}
